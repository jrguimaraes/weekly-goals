...
function CreateWeekFormContent({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="week-start-date" className="block text-xs font-semibold text-slate-700">
      ...
      {calculatedEndDate && (
        <div className="rounded-lg bg-indigo-50/70 p-4 border border-indigo-100 text-xs text-indigo-900 space-y-1">
          <div className="font-semibold text-indigo-950">Intervalo do Ciclo Semanal (7 dias):</div>
          <div className="text-sm font-medium">
            {formatDate(startDate)} &rarr; {formatDate(calculatedEndDate)}
          </div>
          <div className="text-xs text-indigo-700 pt-1">
